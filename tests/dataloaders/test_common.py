...
def test_prepare_ydata():
    ...
    img_id_list, grid_x_list, grid_y_list, box_id_list = np.where(np.sum(y_data_prepared[0], axis=-1) > 0)

    for img_id, grid_x, grid_y, box_id in zip(img_id_list, grid_x_list, grid_y_list, box_id_list):
